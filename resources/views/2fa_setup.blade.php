...
<div class="row">
  <div class="col-md-4 offset-4 mt-5 register">
    ...
    <img class="ms-5" src="data:image/svg+xml;base64,{{ $qrImage }}"/>
    <p>Or use the secret key:<strong> {{ $secret }}</strong></p>
    
    <form action="/2fa/enable" method="post">
      @csrf
      <label for="otp">Enter the 6-digit code:</label>
      <input type="text" name="otp" class="rounded" required>
      <button class="mt-3 ms-5 btn btn-dark text-light" type="submit">Enable 2FA</button>
    </form> 
  </div>
</div> 
